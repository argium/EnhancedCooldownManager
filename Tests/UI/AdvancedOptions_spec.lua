...
describe("AdvancedOptions getters/setters/defaults", function()
    ...
    describe("warnings", function()
        it("getter returns profile value", function()
            assert.is_false(settings["ECM_global_warnings"]:GetValue())
        end)
        it("setter writes to profile", function()
            settings["ECM_global_warnings"]:SetValue(true)
            assert.is_true(profile.global.warnings)
        end)
        it("default matches expected", function()
            assert.is_false(settings["ECM_global_warnings"]._default)
        end)
    end)
