...
| **Options file(s)** | [`UI/ExtraIconsOptions.lua`](../UI/ExtraIconsOptions.lua), [`UI/ItemSetsOptions.lua`](../UI/ItemSetsOptions.lua) |
| **Options dependencies** | <ul><li>`ns.OptionUtil` — enable/disable helpers, shared action buttons, and popup dialog plumbing.</li><li>`LibSettingsBuilder` section-list row (`type = "sectionList"`) — viewer collections, item set lists, and inline add rows.</li><li>Async item-info flow backed by `GET_ITEM_INFO_RECEIVED` and `C_Item.RequestLoadItemDataByID`.</li></ul> |
