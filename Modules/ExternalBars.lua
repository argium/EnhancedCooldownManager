...
function ExternalBars:OnExternalAurasUpdated(reason)
    ...
    local function bail(logKey, message, err)
        activeAuraCount = 0
        wipe(auraStates)
        self:_hideExcessBars(0)
        self:_StopDurationTicker()
    ...
    if type(auraInfo) == "table" and not canAccessTable(auraInfo) then
        bail("AuraInfoInaccessible", "Blizzard external aura info is inaccessible during "
    ...
    elseif type(auraInfo) == "table" then
        ...
        if not ok then
            bail("AuraInfoIterateFailed", "Blizzard external aura info could not be iterated during "
